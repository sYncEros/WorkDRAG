...
class EventViewerAudit:
    ...
    def _query_log(self, log_name: str, event_ids: list[int], source_label: str) -> EventQueryResult | None:
        ...
        log_ps = log_name.replace("'", "''")   # evita romper / inyectar si el nombre lleva comillas
        filter_expr = (
            f"@{{LogName='{log_ps}'; Id=@({ids_ps})}}"
            if self.SCAN_SCOPE == "full"
            else f"@{{LogName='{log_ps}'; Id=@({ids_ps}); StartTime=(Get-Date).AddHours(-{self.HOURS_BACK})}}"
        )

        if self.MAX_EVENTS and self.MAX_EVENTS > 0:
            events_expr = f"$events=Get-WinEvent -FilterHashtable {filter_expr} -MaxEvents {int(self.MAX_EVENTS)} -ErrorAction Stop; "
        else:
            events_expr = f"$events=Get-WinEvent -FilterHashtable {filter_expr} -ErrorAction Stop; "
        ...
        ps = "".join([
            ...
            # TimeCreated formateado ISO 8601 en origen => sin /Date(ms)/ en el JSON
            "$samples=$events | Select-Object -First 15 "
            "@{N='TimeCreated';E={ if($_.TimeCreated){$_.TimeCreated.ToString('o')} else {$null} }}, "
            "Id, ProviderName, MachineName, LevelDisplayName; ",
            "$result=[ordered]@{",
        ...
        try:
            ...
            if result.returncode != 0 or not result.stdout.strip():
                ...
                self.query_errors[source_label] = err
                return EventQueryResult(log_name, 0, {}, [], source_label, accessible=False, error=err)
            ...
            return EventQueryResult(
                ...
                by_id, samples, source_label, accessible=True, error="",
            )
        except subprocess.TimeoutExpired:
            err = f"timeout (45s) consultando {log_name}"
            self.query_errors[source_label] = err
            return EventQueryResult(log_name, 0, {}, [], source_label, accessible=False, error=err)
    ...
    def _export_full_logs_report(self):
        ...
        ps = "\n".join([
            ...
            # UTF-8 SIN BOM (Encoding.UTF8 de .NET sí emite BOM -> era el origen del BOM)
            ...
            "    if ($since) { $filter['StartTime'] = $since }",   # ventana temporal
            "    $events = $null",
            "    try {",
            ...
            "        time_created= if ($evt.TimeCreated) { $evt.TimeCreated.ToString('o') } else { $null }",  # ISO 8601
            ...
            "      $entry.sha256 = (Get-FileHash -Algorithm SHA256 -Path $outFile).Hash",   # integridad
            "    } else { Remove-Item $outFile -ErrorAction SilentlyContinue }",            # no dejar ficheros vacíos
            "  } catch {",
            ...
            # Manifest/suspicious también SIN BOM (Set-Content -Encoding UTF8 mete BOM en PS 5.1)
        ...
        try:
            ...
            # Antes esto colgaba sin fin; ahora corta y deja constancia.
        ...
        # El export en sí es informativo; reservamos color para la señal real.
        ...
        self.engine.add_finding(AuditFinding(
            ...
            raw_data={
                ...
                # Para enriquecer el informe con SEÑAL (no volumen), pasar logs_dir a
                # event_log_summary.summarize_dir() y adjuntar el resultado aquí.
            },
        ))
