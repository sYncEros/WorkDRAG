...
SECURITY_EVENT_IDS = {
    1102: "Registro de auditoría de SEGURIDAD borrado",   # prioridad forense
...
def summarize(path, max_samples=3):
    ...
    with open(path, "r", encoding="utf-8-sig") as fh:   # utf-8-sig => come el BOM
        for line in fh:
            line = line.strip()
            if not line:
                continue
            total += 1
            try:
                r = json.loads(line)                     # línea mala -> se salta, no aborta
            except Exception:
                failed += 1
                continue
            parsed += 1
...
def summarize_dir(directory):
    ...
    for f in sorted(directory.glob("*.ndjson")):
        ...
        except Exception as e:  # un fichero ilegible no tumba el resto
