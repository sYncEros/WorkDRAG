...
from skills.event_logs.log_query import query_log
...
class EventLogMonitor:
    ...
    def _check_usb_events(self):
        """Consulta eventos USB relevantes (2003/2100) reutilizando infraestructura compartida."""
        targets = [
            ("Microsoft-Windows-DriverFrameworks-UserMode/Operational", [2003, 2100], "usb_dfu"),
            ("Microsoft-Windows-Kernel-PnP/Configuration", [400, 410, 430], "usb_kernelpnp"),
        ]
        ...
        for log_name, ids, label in targets:
            res = query_log(
                log_name,
                ids,
                label,
            ...
            matches = int(sum(res.by_id.values()))
            if matches <= 0:
                continue
            ...
            all_samples.extend(res.samples[:10])
        ...
        if total:
            print(
                f"[EventLog] USB events detectados: {total} "
                f"en {len(by_log)} logs"
            )
    ...
    def _report(self):
        ...
        # ── Hallazgo 6: Eventos USB en Event Viewer ─────────────────────────
        if self.usb_events.get("total", 0) > 0:
            self.engine.add_finding(AuditFinding(
                skill=self.SKILL_NAME,
                category="usb_event_log_activity",
                title=(
                    "Actividad USB en Event Logs "
                    f"({self.usb_events.get('total', 0)} eventos)"
                ),
                description=(
                    "Se detectaron eventos USB en canales de Event Viewer "
                    "relevantes para trazabilidad forense (conexión/configuración)."
                ),
                risk_level="yellow",
                technical_risk=(
                    "Los eventos USB pueden evidenciar intentos de conexión de "
                    "dispositivos, cambios de estado y actividad potencial de "
                    "transferencia física de datos."
                ),
                legal_risk=(
                    "Estos registros son relevantes para acreditar controles DLP "
                    "sobre periféricos y posibles medidas disciplinarias basadas "
                    "en actividad de dispositivos externos."
                ),
                what_it_is=(
                    "Correlación de eventos USB desde logs operacionales de Windows "
                    "en una ventana de 30 días."
                ),
                what_it_is_not=(
                    "No demuestra por sí solo exfiltración de datos; describe señales "
                    "de actividad de dispositivos USB."
                ),
                raw_data=self.usb_events,
            ))
        ...
        print(
            ...
            f"USB events: {self.usb_events.get('total', 0)}"
        )
