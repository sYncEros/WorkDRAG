...
def hacer_bloque_hallazgo(titulo, descripcion, accion, risk_level, styles):
    """Hallazgo como tabla — se adapta al contenido sin truncar."""
    color_map = {
        "red": ROJO, "orange": NARANJA,
        "yellow": AMARILLO, "green": VERDE,
    }
    color = color_map.get(risk_level, GRIS_MED)

    titulo_p = Paragraph(titulo, ParagraphStyle(
        "ht", fontName="Helvetica-Bold", fontSize=9,
        textColor=GRIS_DARK, spaceAfter=2,
    ))
    desc_p = Paragraph(descripcion, ParagraphStyle(
        "hd", fontName="Helvetica", fontSize=8,
        textColor=GRIS_MED, leading=12, spaceAfter=3,
    ))
    partes = [titulo_p, desc_p]
    if accion:
        partes.append(Paragraph(f"→ {accion}", ParagraphStyle(
            "ha", fontName="Helvetica-Bold", fontSize=8,
...
CATEGORIA_HUMANA = {
    ...
    # ── Nuevas ────────────────────────────────────────────────────────────────
    "browser_inspection":           ("Hay extensiones de navegador instaladas por la empresa",
                                     "Pide a IT la función exacta de cada extensión forzada"),
    "surveillance_capability_signals": ("Se detectan capacidades de monitorización a nivel de sistema",
                                     "Pide al DPO qué software de monitorización está autorizado"),
    "persistence_untrusted_certs":  ("Hay certificados corporativos que permiten descifrar tu tráfico HTTPS",
                                     "No uses el equipo para comunicaciones privadas"),
    "privacy_clipboard":            ("Hay apps que pueden leer lo que copias al portapapeles",
                                     "No copies contraseñas ni datos personales en este equipo"),
    "telemetry_office":             ("Office recopila datos de uso de tus documentos y los envía a Microsoft",
                                     "Pide al DPO qué datos envía Office y bajo qué base legal"),
    "cloud_sync_service":           ("SharePoint y OneDrive están activos sincronizando archivos",
                                     "No guardes archivos personales en carpetas corporativas"),
    "browser_policies":             ("La empresa controla la configuración de tu navegador",
                                     "Pide a IT qué políticas están activas y para qué"),
    "hardening_boot":               ("El arranque seguro no está activo — el equipo es vulnerable",
                                     "Pide a IT que activen Secure Boot"),
    "hardening_antimalware":        ("La protección antivirus en tiempo real no está activa",
                                     "Pide a IT que activen Windows Defender"),
    "identity_local_accounts":      ("Hay cuentas locales en tu equipo sin documentar",
                                     "Solicita inventario oficial de cuentas y su justificación"),
    "identity_admin_group":         ("Hay 4 cuentas con acceso de administrador a tu equipo",
                                     "Solicita listado oficial de admins y su función"),
    "identity_account_profiles":    ("Hay cuentas con alertas de seguridad activas",
                                     "Pide que IT deshabilite las cuentas no necesarias"),
    "imposed_identity":             ("La identidad de Git no corresponde a tu usuario",
                                     "Verifica que tu identidad Git es correcta"),
    "third_party_apps_policies":    ("La empresa controla qué apps puedes usar mediante GPO",
                                     "Pide listado de políticas de aplicaciones activas"),
    "exfiltration_dlp_monitoring":  ("Hay software DLP activo que puede inspeccionar tus archivos",
                                     "Pide al DPO qué datos inspecciona el DLP y con qué finalidad"),
    "event_viewer_sensitive_events":("Hay eventos sensibles registrados en el visor de eventos",
                                     "Pide al DPO quién tiene acceso a estos registros"),
    "rdp_telemetry_coverage_gap":   ("No se pueden ver todos los accesos remotos por falta de permisos",
                                     "Ejecuta la auditoría como administrador para cobertura completa"),
    "dpa_office_no_policy":         ("Office no tiene política de privacidad — envía datos por defecto",
                                     "Pide a IT que apliquen política GPO de privacidad de Office"),
    "hardening_missing":            ("Faltan configuraciones de seguridad básicas en tu equipo",
                                     "Pide a IT que apliquen las configuraciones ausentes"),
    "incident_response_playbook":   ("Existe un protocolo de investigación que puede acceder a tus datos",
                                     "Solicita al DPO qué datos se acceden durante una investigación"),
    "diagtrack_active":             ("DiagTrack está activo enviando todos tus datos a Microsoft",
                                     "Pide a IT que lo desactiven — está bloqueado por GPO corporativa"),
    "diagtrack_no_policy":          ("No hay política que limite la telemetría de Windows",
                                     "Pide a IT que apliquen AllowTelemetry=1 mediante GPO"),
}
def _titulo_humano(finding: dict) -> tuple[str, str]:
    ...
    for term in [
        " — sin restricción GPO", " — GPO restrictiva activa",
        " — desactivación BLOQUEADA por GPO", " — bloqueo activo",
        " detectado", " activo", " habilitado", " configurado",
        " — Urgencia: INMEDIATA (< 24 horas)",
    ]:
        titulo = titulo.replace(term, "")

    # Eliminar patrones con números técnicos tipo "(drivers filtro=28, SYSTEM->ex"
    import re
    titulo = re.sub(r'\s*\(drivers.*', '', titulo)
    ...
    # Acción — primera recomendación si existe
    accion = ""
    recs = finding.get("recommendations", [])
    if recs and isinstance(recs, list):
        accion = recs[0][:80] if recs[0] else ""

    return titulo.strip(), accion
...
def export_pdf_trabajador(
    ...
    story.append(Paragraph(
        f"Auditoría realizada el {fecha} · Sistema WorkDRAG",
        st["subtitulo"]
    ))
    story.append(Spacer(1, 4*mm))
    ...
    if rojos:
        ...
        for f in rojos[:8]:  # máximo 8 para no saturar
            ...
            story.append(hacer_bloque_hallazgo(
                titulo_h,
                f.get("description", ""),   # ← sin truncar
                accion_h,
                "red",
                st,
            ))
